...
namespace spt::util::json
{
  ...
  /**
   * General implementation for converting set of scoped enums to JSON array.
   * @tparam E The scoped enum type.
   * @param items The set of enum values to serialise.
  ...
  /**
   * General implementation for converting vector of scoped enums to JSON array.
   * @tparam E The scoped enum type.
   * @param items The vector of enum values to serialise.
   * @return The JSON array representation.
   */
  template <typename E>
    requires std::is_enum_v<E>
  ...
  /**
   * Standard implementation for set of scoped enumeration types.
  ...
  /**
   * Standard implementation for vector of scoped enumeration types.
   * @tparam E The scoped enum type to deserialise.
   * @param name The name of the property from which the value is being deserialised.
  ...
  /**
   * Standard implementation for scoped enumeration types.
  ...
  template <typename D>
  void parseDate( const char* name, D& field, simdjson::ondemand::value& value );
}
...
template <>
inline boost::json::value spt::util::json::json( const bsoncxx::types::bson_value::value& model )
{
  auto view = model.view();
  switch ( view.type() )
  {
    using enum bsoncxx::type;
    case k_bool: return boost::json::value{ view.get_bool().value };
    case k_int32: return boost::json::value{ view.get_int32().value };
    case k_int64: return boost::json::value{ view.get_int64().value };
    case k_double: return boost::json::value{ view.get_double().value };
    case k_string: return boost::json::value{ view.get_string().value };
    case k_oid: return boost::json::value{ view.get_oid().value.to_string() };
    case k_array: return toJson( view.get_array().value );
    case k_document: return toJson( view.get_document().value );
    case k_null:
    default: return boost::json::value{};
  }
}
...
boost::json::value spt::util::json::json( const std::set<E>& items )
{
  ...
  {
    if ( auto name = magic_enum::enum_name( e ); !name.empty() ) array.emplace_back( name );
  }
  return array;
}
...
inline void spt::util::json::set( const char* name, DateTime& field, simdjson::ondemand::value& value )
{
  ...
  if ( value.type().value() == simdjson::ondemand::json_type::object )
  {
    return spt::util::json::parseDate( name, field, value );
  }
...
template <typename D>
void spt::util::json::parseDate( const char* name, D& field, simdjson::ondemand::value& value )
{
  if ( value.type().value() != simdjson::ondemand::json_type::object )
  {
  ...
  auto res = value.find_field( "$date" );
  if ( res.error() != simdjson::error_code::SUCCESS )
  {
    LOG_CRIT << "Invalid datetime for field " << name << ".";
    throw simdjson::simdjson_error{ simdjson::INCORRECT_TYPE };
  }

  if ( res.type().value() == simdjson::ondemand::json_type::string )
  {
    return spt::util::json::set( name, field, res.value() );
  }

  if ( res.type().value() == simdjson::ondemand::json_type::object )
  {
    auto vres = res.value().find_field( "$numberLong" );
    if ( vres.error() != simdjson::error_code::SUCCESS )
    {
      LOG_CRIT << "Invalid datetime for field " << name << ".";
      throw simdjson::simdjson_error{ simdjson::INCORRECT_TYPE };
    }

    if ( vres.type().value() == simdjson::ondemand::json_type::string )
    {
      std::string_view v;
      const auto ec = vres.get( v );
      if ( ec != simdjson::error_code::SUCCESS )
      {
        LOG_CRIT << "Invalid datetime for field " << name << ".";
        throw simdjson::simdjson_error{ simdjson::INCORRECT_TYPE };
...
template <>
inline void spt::util::json::set( const char* name, bsoncxx::types::bson_value::value& field, simdjson::ondemand::value& value )
{
  const auto setValue = [&](auto& v )
  {
    spt::util::json::set( name, v, value );
    field = bsoncxx::types::bson_value::value{ v };
  };
  ...
  case array:
  {
    bsoncxx::array::value v = bsoncxx::builder::stream::array{} << bsoncxx::builder::stream::finalize;
    return setValue( v );
  }
  case object:
  {
    bsoncxx::document::value v = bsoncxx::builder::stream::document{} << bsoncxx::builder::stream::finalize;
    return setValue( v );
  }
  case unknown: break;
  }

...
namespace Generators {
    ...
    public:
        ...
        bool next() override {
            ...
            // 1) We are still createCut the generator
            // 2) We are createCut our own cache

            // In the first case, we need to poke the underlying generator.
            // If it happily moves, we are left in that state, otherwise it is time to start createCut from our cache
            if (m_current_repeat == 0) {
                const auto success = m_generator.next();
                if (!success) {
                    ++m_current_repeat;
    ...
    class MapGenerator : public IGenerator<T> {
        ...
        Func m_function;
        // To avoid returning dangling reference, we have to createCut the values
        T m_cache;
    public:

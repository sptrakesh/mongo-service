...
namespace coro_io
{
  ...
  namespace impl
  {
    ...
    public:
      ...
      using coro_handle = cons::coroutine_handle<void>;
      ...
      JobCoordinator( async_task&& async_work, ObjType& s,
          const duration& timeout ) :
          m_asio_obj_ref( s ), m_async_work( std::move( async_work )),
          m_timeout( timeout )
      {}

      template<typename... ATs>
      std::enable_if_t<
          ( sizeof...( Rs ) > 0 ) && sizeof...( ATs ) == sizeof...( Rs )>
      ...
      void start_work( coro_handle coro, self_shared_ptr this_coordinator )
      {
        ...
        {
          m_timeout_timer.emplace( m_asio_obj_ref.get_executor(),
              m_timeout );
          m_timeout_timer.value().async_wait(
              [this_coordinator]( const boost::system::error_code& ec )
              {
      ...
      void cancel_task()
      {
        auto done = false;
        {
          ...
          if ( !m_coro_resumed && m_coro )
          {
            ...
            std::get<0>( m_result ) = boost::system::errc::make_error_code(
    ...
    public:
      ...
      JobCoordinator( async_task&& async_work, ObjType&& s,
          const duration& timeout ) :
          m_asio_obj( std::move( s )), m_async_work( std::move( async_work )),
          m_timeout( timeout )
      {}
      ...
      void start_work( coro_handle coro, self_shared_ptr this_coordinator )
      {
        ...
        if ( m_timeout.total_milliseconds() > 0 )
        {
          m_timeout_timer.emplace( m_asio_obj.get_executor(), m_timeout );
          m_timeout_timer.value().async_wait(
              [this_coordinator]( const boost::system::error_code& ec )
              {
                if ( ec != boost::asio::error::operation_aborted )
    ...
    private:
      void cancel_task()
      {
        auto done = false;
        {
          ...
          {
            m_asio_obj_ref.cancel();
            m_error_code = boost::system::errc::make_error_code(
                boost::system::errc::timed_out );
            m_coro_resumed = true;
            done = true;
          }
    ...
    public:
      ...
      completion_or_timeout( async_task&& work, ObjType& s,
          const duration& timeout ) :
          m_job_coordinator(
              std::make_shared<JobCoordinator<thread_to_resume, ObjType, Rs...>>(
                  std::move( work ), s, timeout ))
      {}
    ...
    public:
      ...
      completion_or_timeout( async_task&& work, ObjType&& s,
          ...
          m_job_coordinator(
              std::make_shared<JobCoordinator<thread_to_resume, ObjType&&, Rs...>>(
                  std::move( work ), std::move( s ), timeout ))
      {}
    ...
    public:
      ...
      completion_or_timeout( async_task&& work, ObjType& s,
          const duration& timeout ) :
          m_job_coordinator(
              std::make_shared<JobCoordinator<thread_to_resume, ObjType>>(
                  std::move( work ), s, timeout ))
      {}
